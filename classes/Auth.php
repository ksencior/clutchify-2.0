...
class Auth {
    ...
    public function register(string $username, string $email, string $password): array {
        ...
        if ($stmt->fetch()) {
            return ['success' => false, 'message' => 'Użytkownik z podanym nickiem / e-mailem już istnieje!'];
        }
        ...
        try {
            ...
            $stmtUser = $this->db->prepare('INSERT INTO users (username, email, password) VALUES (?, ?, ?)');
            $stmtUser->execute([$username, $email, $hashedPassword]);

            $userID = $this->db->lastInsertId();

            $stmtPlayer = $this->db->prepare("INSERT INTO players (user_id) VALUES (?)");
            ...
            $this->db->rollBack();
            return ['success' => false, 'message' => 'Wystąpił błąd. Spróbuj ponownie później. ' . $e->getMessage()];
        }
    }
